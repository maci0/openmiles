...
pub fn AIL_redbook_close(hb: ?*openmiles.Redbook) callconv(.winapi) void {
    const rb = hb orelse return;
    log("AIL_redbook_close(hb={*})\n", .{rb});
    rb.deinit();
}
pub fn AIL_redbook_play(hb: ?*openmiles.Redbook, start: u32, end: u32) callconv(.winapi) u32 {
    const rb = hb orelse return 0;
    log("AIL_redbook_play(hb={*}, start={d}, end={d})\n", .{ rb, start, end });
    rb.play(start, end);
    return 1;
}
pub fn AIL_redbook_stop(hb: ?*openmiles.Redbook) callconv(.winapi) u32 {
    const rb = hb orelse return 0;
...
pub fn AIL_redbook_position(hb: ?*openmiles.Redbook) callconv(.winapi) u32 {
    const rb = hb orelse return 0;
    return rb.getPosition();
}
pub fn AIL_redbook_track(hb: ?*openmiles.Redbook) callconv(.winapi) u32 {
    const rb = hb orelse return 0;
    return rb.current_track;
}
pub fn AIL_redbook_track_info(hb: ?*openmiles.Redbook, track: u32, start_ms: ?*u32, end_ms: ?*u32) callconv(.winapi) void {
    _ = hb;
    _ = track;
    if (start_ms) |p| p.* = 0;
    if (end_ms) |p| p.* = 0;
