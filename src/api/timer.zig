...
pub fn AIL_register_timer(callback: *const fn (u32) callconv(.winapi) void) callconv(.winapi) ?*anyopaque {
...
pub fn AIL_set_timer_frequency(timer_opt: ?*openmiles.Timer, hertz: u32) callconv(.winapi) void {
...
pub fn AIL_set_timer_period(timer_opt: ?*openmiles.Timer, us: u32) callconv(.winapi) void {
    const timer = timer_opt orelse return;
    log("AIL_set_timer_period(timer={*}, us={d})\n", .{ timer, us });
    timer.setPeriodUs(@as(u64, us));
}
pub fn AIL_set_timer_user_data(timer_opt: ?*openmiles.Timer, data: u32) callconv(.winapi) void {
    const timer = timer_opt orelse return;
    log("AIL_set_timer_user_data(timer={*}, data={d})\n", .{ timer, data });
    timer.setUserData(data);
}
pub fn AIL_start_timer(timer_opt: ?*openmiles.Timer) callconv(.winapi) void {
    const timer = timer_opt orelse return;
    log("AIL_start_timer(timer={*})\n", .{timer});
    timer.start();
}
pub fn AIL_stop_timer(timer_opt: ?*openmiles.Timer) callconv(.winapi) void {
    const timer = timer_opt orelse return;
    log("AIL_stop_timer(timer={*})\n", .{timer});
    timer.stop();
}
pub fn AIL_release_timer_handle(timer_opt: ?*openmiles.Timer) callconv(.winapi) void {
    const timer = timer_opt orelse return;
    log("AIL_release_timer_handle(timer={*})\n", .{timer});
    timer.deinit();
}
pub fn AIL_start_all_timers() callconv(.winapi) void {
    log("AIL_start_all_timers()\n", .{});
    openmiles.startAllTimers();
}
pub fn AIL_stop_all_timers() callconv(.winapi) void {
